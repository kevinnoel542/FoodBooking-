<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(Auth::user()->role === 'admin')
                        <x-nav-link href="{{ route('admin.foods.index') }}" :active="request()->routeIs('foods.*')">
                            {{ __('Manage Foods') }}
                        </x-nav-link>
                        <x-nav-link href="{{ route('admin.bookings.index') }}" :active="request()->routeIs('bookings.*')">
                            {{ __('Manage Bookings') }}
                        </x-nav-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(Auth::user()->role === 'admin')
                <x-responsive-nav-link href="{{ route('admin.foods.index') }}" :active="request()->routeIs('foods.*')">
                    {{ __('Manage Foods') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link href="{{ route('admin.bookings.index') }}" :active="request()->routeIs('bookings.*')">
                    {{ __('Manage Bookings') }}
                </x-responsive-nav-link>
            @elseif(Auth::user()->role === 'user')
